...
--SELECT cod, design FROM cadeira WHERE curso LIKE "AC";
--SELECT aluno.nome FROM aluno, prof WHERE aluno.nome = prof.nome;
--SELECT aluno.nome FROM aluno WHERE NOT EXISTS (SELECT aluno.nome FROM prof WHERE aluno.nome = prof.nome);
--SELECT nome FROM aluno UNION SELECT nome FROM prof;
--SELECT DISTINCT nome FROM aluno NATURAL JOIN prova WHERE prova.cod LIKE 'TS1';
--SELECT DISTINCT nome FROM aluno NATURAL JOIN (SELECT prova.nr FROM prova, cadeira WHERE prova.cod = cadeira.cod AND cadeira.curso LIKE 'IS');
-- Couldn't do 8
--SELECT DISTINCT p1.nota FROM prova p1 WHERE NOT EXISTS (SELECT * from prova p2 WHERE p2.nota > p1.nota)
--SELECT avg(nota) FROM prova WHERE prova.cod LIKE 'BD';
--SELECT count(nr) FROM aluno;
--SELECT curso, count(cod) FROM cadeira group by curso;
--SELECT nr, count(nr) FROM prova group by nr;
--SELECT avg(countnr) FROM (SELECT count(nr) as countnr FROM prova group by nr);
--SELECT nr, avg(nota) FROM (SELECT * FROM prova WHERE nota >= 10) group by nr;
--SELECT cod, MAX(nota) FROM prova group by cod;
-- Couldn't do 17
